...
class Repository
{
    ...
    async chapterFromDirectory(chapterPath)
    {
        ...
        if ( await this.isChapterDirectory(chapterPath) )
        {
            ...
            log(`Determining path relative to repository root`);
            const rootDirectory = await this.rootDirectory;
            const relativePath = path.relative(rootDirectory, chapterPath);
            log(`Relative path found: ${relativePath}`);

            log(`Checking relative path ${relativePath} for validity`);
            if ( /[/]/.exec(relativePath) )
            {
                console.err(`Relative path ${relativePath} cannot be used as chapter id`);
                process.exit(-1);
            }
            else
            {
...
async function runTests(options)
{
    ...
    {
        log(`Running tests in current directory ${process.cwd()}`);
